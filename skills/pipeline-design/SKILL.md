...
description: Recipe the ai-agents-architect follows to mint a named pipeline within a chapter. Defines the pipeline file schema (phases, exit criteria, document handoffs, gates, iteration loops with bounds, severity protocol, rules, failure modes), the design heuristics that come from a real-world agentic pipeline pattern, and the don'ts. Invoke when designing any new pipeline.
---

# Pipeline design recipe

A **pipeline** is a named, phase-structured workflow within a chapter. Where the chapter is the *team*, the pipeline is the *playbook* the team runs against a specific kind of work. A chapter can have zero, one, or many pipelines — they're additive to the roster, never required.

A pipeline file lives at `.claude/agents/<discipline>/pipelines/<pipeline-name>.md` of the consuming project.

## When to mint a pipeline (vs. leaving routing ad-hoc)

Mint a pipeline when **all** of these hold:
...
If only one or two hold, leave the work to the orchestrator's ad-hoc routing. Pipelines have overhead — phase boundaries, artifact files, state tracking — that pays back only when the structure is reused.

## The pipeline file schema

YAML frontmatter for the structured spine, markdown body for the prose contracts:

```markdown
---
name: <pipeline-name>                       # discipline-prefixed kebab-case, e.g. "engineering-feature-implement"
discipline: <chapter-name>
version: <semver>                           # bump on revision so the manager can detect stale state
description: <one sentence — what this pipeline does end-to-end>
run_artifacts_dir: .claude/pipeline-runs/   # default; override per pipeline if you need a different audit trail location
...
### Mandatory (CR-*)
- CR-1: <rule that must hold for the pipeline's output to be valid>
- CR-2: <rule>

### Advisory (AR-*)
- AR-1: <rule the specialists should respect but won't halt the pipeline if violated>
...
- `[BLOCKING]` — halt the pipeline (or the loop), surface to human.
...
Reference roster members by name. Same chapter: bare name (`engineering-spec-author`). Cross-chapter: `<chapter>/<specialist>` (e.g. `marketing/copy-reviewer`). Cross-chapter references are first-class — pipelines can pull a specialist from another chapter when the work crosses disciplines.
...
- **Mandatory (CR-*)** — must hold for the pipeline's output to be valid. Specialists cite the CR-* numbers they depend on or violate. Verify-phase specialists check CR-* compliance.
...
When designing a pipeline, lean on these:
...
7. **Be transparent about limitations.** If a phase can't reliably catch a class of issue (e.g. visual regressions Visual Reviewer misses), say so in the phase's body. Don't pretend the pipeline is infallible.

## Common pipeline patterns

### 3-phase Understand → Build → Verify

The article's design-system component pipeline. Generalizes to: any task where you research/design first, build second, validate third. Engineering features, marketing campaigns, security audits.

### 5-phase incident response

Detect → Contain → Diagnose → Remediate → Postmortem. Distinct from the 3-phase pattern because each phase has a different *time pressure* — Detect/Contain are minutes; Diagnose is hours; Remediate is days; Postmortem is the week after. Pipeline encodes the temporal shape.

### 4-phase validate-build

Validate (problem, market, unit economics) → Plan (scope, milestones, team) → Build → Verify. For business-validation and product-launch pipelines.

### N-phase pipelines

Rare. Most "long" pipelines decompose into nested 3-phase pipelines or are over-described. If you find yourself writing a 9-phase pipeline, ask whether phases 4–7 are actually one Build phase in disguise.
...
- **Don't loop without a cap.** Every loop must have a soft cap that escalates to a gate. Unbounded loops are how pipelines burn down a token budget overnight.
...
- **Don't bake project specifics into the pipeline.** A pipeline is a *shape*. Specifics (file paths, library names, token sets) belong in the specialists' system prompts or in `CLAUDE.md`. If the pipeline file mentions `next.config.ts`, you're making it brittle.
- **Don't create a pipeline for a one-off task.** If you'd run this exactly once, route ad-hoc. Pipelines pay off when reused.
- **Don't reference specialists that don't exist in the chapter's roster.** The architect should mint missing specialists *first*, then design the pipeline. The orchestrator can't dispatch agents that don't exist.
